...
// @desc    Xóa ảnh bìa (cover)
// @route   DELETE /api/users/me/cover
// @access  Private
const deleteCover = asyncHandler(async (req, res) => {
  const userId = req.user.id;
  const user = await userService.deleteUserCover(userId);
...
// @desc    Cập nhật social links
// @route   PUT /api/users/me/social-links
// @access  Private
const updateSocialLinks = asyncHandler(async (req, res) => {
  const userId = req.user.id;
  const socialLinks = req.body;
  
  const user = await userService.updateUserSocialLinks(userId, socialLinks);
