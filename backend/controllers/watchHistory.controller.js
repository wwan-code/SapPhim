...
export const getProgress = async (req, res, next) => {
  try {
    ...
    if (!movieId) {
      return sendResponse(res, 400, false, null, 'movieId là bắt buộc.');
    }

    const episodeIdInt = episodeId ? parseInt(episodeId, 10) : null;
    const movieIdInt = parseInt(movieId, 10);

    const record = await watchHistoryService.getWatchProgress({
      userId,
      movieId: movieIdInt,
    ...
    if (!record) {
      return sendResponse(res, 404, false, null, 'Không tìm thấy lịch sử xem cho phim/tập này.');
    }

    sendResponse(res, 200, true, record, 'Lấy tiến độ xem thành công.');
  } catch (error) {
    next(error);
  }
};
