...
const getMovies = asyncHandler(async (req, res) => {
  ...
  const result = await redisHelpers.cache(cacheKey, async () => {
    return await movieService.getMovies(req.query);
  }, 300); // Cache 5 minutes
...
const getMovieDetail = asyncHandler(async (req, res) => {
  ...
  // Cache movie detail (excluding user-specific data like isFavorite)
  const cacheKey = `movie:detail:${slug}`;
  const movieData = await redisHelpers.cache(cacheKey, async () => {
    const { movie } = await movieService.getMovieDetailBySlug(slug, null);
  ...
  if (movieData) {
    ...
    if (userId) {
      // Check favorite status directly from DB or separate cache
      // For now, we can re-use service method or optimize later
      const { isFavorite: favStatus } = await movieService.getMovieDetailBySlug(slug, userId);
      isFavorite = favStatus;
    }

    res.status(200).json({
      success: true,
      data: { ...movieData, isFavorite },
    });
  } else {
    res.status(404);
    throw new Error('Phim không được tìm thấy.');
