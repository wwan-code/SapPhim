...
const register = asyncHandler(async (req, res) => {
  ...
  const { user, accessToken, refreshToken } = await authService.registerUser(username, email, password, phoneNumber, req);
...
const socialLogin = asyncHandler(async (req, res) => {
  ...
  const { user, accessToken, refreshToken, isNewUser } = await authService.socialLogin(idToken, provider, req);
  ...
  res.status(200).json({
    ...
    message: isNewUser 
      ? `Chào mừng! Tài khoản của bạn đã được tạo thành công qua ${provider}.`
      : `Đăng nhập bằng ${provider} thành công.`,
    errors: null,
    meta: null,
  });
});
