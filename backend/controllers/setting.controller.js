...
// @desc    Hủy một phiên đăng nhập cụ thể
// @route   DELETE /api/account/login-history/:id
// @access  Private
const revokeLoginSession = asyncHandler(async (req, res) => {
  const userId = req.user.id;
  const loginHistoryId = req.params.id;
  
  await settingService.revokeLoginSession(userId, loginHistoryId);
...
// @desc    Hủy tất cả phiên đăng nhập khác
// @route   POST /api/account/revoke-all-sessions
// @access  Private
const revokeAllOtherSessions = asyncHandler(async (req, res) => {
  const userId = req.user.id;
  
  await settingService.revokeAllOtherSessions(userId);
  
  res.status(200).json({
    data: null,
    message: 'Đã hủy tất cả phiên đăng nhập khác thành công.',
    errors: null,
    meta: null,
  });
});
