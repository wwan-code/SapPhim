...
const adminApiKey = process.env.GEMINI_ADMIN_API_KEY || process.env.GEMINI_API_KEY;
const genAI_Admin = new GoogleGenerativeAI(adminApiKey);
const model_Admin = genAI_Admin.getGenerativeModel({ model: "gemini-2.5-flash" });
...
- **TIẾNG VIỆT**: Phản hồi bằng tiếng Việt.`,
    ...
    CHAT_INTENT_CLASSIFIER: {
        template: `Bạn là một hệ thống NLU (Natural Language Understanding) của một trợ lý AI cho website xem phim Sạp Phim.
Nhiệm vụ của bạn là phân tích yêu cầu của người dùng và phân loại ý định (intent) cùng các thực thể (entities) liên quan.

NGỮ CẢNH CUỘC TRÒ CHUYỆN (Lịch sử gần đây, không bắt buộc):
{chatHistory}

YÊU CẦU CỦA NGƯỜI DÙNG:
"{prompt}"

CÁC Ý ĐỊNH (INTENTS) HỢP LỆ:
- search_movie: Người dùng muốn tìm kiếm phim (theo tên, diễn viên, đạo diễn, từ khóa).
- recommend_movie: Người dùng muốn được gợi ý phim (chung chung, theo thể loại, tâm trạng, hoặc phim tương tự).
- navigation: Người dùng muốn điều hướng đến một trang cụ thể (phim mới, phim lẻ, trang cá nhân).
- get_movie_details: Người dùng hỏi thông tin chi tiết về MỘT bộ phim cụ thể (nội dung, số tập, năm sản xuất).
- help: Người dùng hỏi về chức năng của trợ lý hoặc cần trợ giúp chung.
- general_chat: Các câu hỏi xã giao, chào hỏi, hoặc câu hỏi không thuộc các loại trên.

CÁC THỰC THỂ (ENTITIES) CẦN TRÍCH XUẤT:
- query: Từ khóa tìm kiếm (tên phim, diễn viên, v.v.)
- genre: Thể loại (ví dụ: "hành động", "kinh dị")
- category: Danh mục (ví dụ: "phim lẻ", "phim bộ")
- releaseYear: Năm phát hành
- target_route: Trang mà người dùng muốn đến (ví dụ: "/phim-moi-cap-nhat", "/profile")

YÊU CẦU TRẢ VỀ (CHỈ JSON):
Trả về MỘT đối tượng JSON duy nhất. KHÔNG GIẢI THÍCH, KHÔNG CHÀO HỎI.

{
  "intent": "INTENT_HỢP_LỆ",
  "entities": {
  ...
  "confidence": 0.9, // Độ tự tin từ 0.0 đến 1.0
  "response_to_user": "Một câu trả lời ngắn gọn (bằng Tiếng Việt) mà AI có thể dùng để bắt đầu, ví dụ: 'Tôi đang tìm phim [query] cho bạn...'"
}

VÍ DỤ:
- Prompt: "Tìm cho tôi phim Inception"
- JSON: { "intent": "search_movie", "entities": { "query": "Inception" }, "confidence": 1.0, "response_to_user": "Đang tìm phim Inception..." }
- Prompt: "Phim lẻ nào hay?"
- JSON: { "intent": "recommend_movie", "entities": { "category": "Phim lẻ" }, "confidence": 0.9, "response_to_user": "Để tôi gợi ý vài phim lẻ hay nhé..." }
- Prompt: "Đưa tôi đến trang phim mới cập nhật"
- JSON: { "intent": "navigation", "entities": { "target_route": "/phim-moi-cap-nhat" }, "confidence": 1.0, "response_to_user": "Đang chuyển bạn đến trang phim mới..." }
- Prompt: "Chào bạn"
- JSON: { "intent": "general_chat", "entities": {}, "confidence": 1.0, "response_to_user": "Chào bạn! Tôi có thể giúp gì?" }

HÃY PHÂN TÍCH YÊU CẦU SAU:
- Lịch sử: {chatHistory}
- Người dùng: "{prompt}"
JSON:`,
        variables: ['prompt', 'chatHistory']
    },
};
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
        try {
            switch (provider) {
                ...
                case AI_PROVIDERS.GEMINI_ADMIN:
                    const adminResult = await model_Admin.generateContent(prompt);
                    return adminResult.response.text();
