...
const updateUserProfile = async (userId, updatedData) => {
  ...
  const { username, email, phoneNumber, sex, bio, avatarUrl, coverUrl } = updatedData;

  if (username) user.username = username;
  if (phoneNumber) user.phoneNumber = phoneNumber;
  if (sex) user.sex = sex;
  if (bio) user.bio = bio;
  // avatarUrl và coverUrl sẽ được xử lý bởi các hàm riêng biệt hoặc khi giá trị là null để xóa
  if (updatedData.hasOwnProperty('avatarUrl') && updatedData.avatarUrl === null) {
  ...
  if (updatedData.hasOwnProperty('coverUrl') && updatedData.coverUrl === null) {
    // Xóa ảnh cũ nếu có và không phải là ảnh mặc định
...
/**
 * @desc Xóa ảnh đại diện (avatar) của người dùng
 * @param {number} userId - ID của người dùng
 * @returns {Promise<User>} Đối tượng người dùng đã được cập nhật
 */
const deleteUserAvatar = async (userId) => {
  ...
  user.avatarUrl = null; // Đặt về null hoặc đường dẫn ảnh mặc định
  await user.save();
  return user;
};
...
/**
 * @desc Xóa ảnh bìa (cover) của người dùng
 * @param {number} userId - ID của người dùng
 * @returns {Promise<User>} Đối tượng người dùng đã được cập nhật
 */
const deleteUserCover = async (userId) => {
  const user = await User.findByPk(userId, {
    include: [{ model: Role, as: 'roles', attributes: ['name'] }],
    attributes: { exclude: ['password', 'updatedAt', 'deletedAt', 'provider'] }
  });
  ...
  user.coverUrl = null; // Đặt về null hoặc đường dẫn ảnh mặc định
  await user.save();
  return user;
};
...
/**
 * @desc Cập nhật social links của người dùng
 * @param {number} userId - ID của người dùng
 * @param {object} socialLinks - Object chứa social links (github, twitter, instagram, facebook)
 * @returns {Promise<User>} Đối tượng người dùng đã được cập nhật
 */
const updateUserSocialLinks = async (userId, socialLinks) => {
  ...
  // Validate và clean up URLs
  const validatedLinks = {};
  const allowedPlatforms = ['github', 'twitter', 'instagram', 'facebook'];

  for (const platform of allowedPlatforms) {
    if (socialLinks && socialLinks[platform] !== undefined) {
      const link = socialLinks[platform]?.trim();
      validatedLinks[platform] = link || '';
    }
  }
