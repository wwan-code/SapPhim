...
/**
 * @desc Xóa một phiên đăng nhập cụ thể
 * @param {number} userId - ID của người dùng
 * @param {number} loginHistoryId - ID của lịch sử đăng nhập cần xóa
 * @returns {Promise<void>}
 */
const revokeLoginSession = async (userId, loginHistoryId) => {
  try {
    const loginHistory = await LoginHistory.findOne({
      where: { id: loginHistoryId, userId },
    });

    if (!loginHistory) {
      throw new Error('Không tìm thấy phiên đăng nhập này.');
    }

    // Cập nhật logoutAt nếu chưa có
    if (!loginHistory.logoutAt) {
    ...
    console.error('Error in revokeLoginSession:', error);
    throw error;
  }
};

/**
 * @desc Xóa tất cả phiên đăng nhập ngoại trừ phiên hiện tại
 * @param {number} userId - ID của người dùng
 * @returns {Promise<void>}
 */
const revokeAllOtherSessions = async (userId) => {
  try {
    ...
    // Cập nhật logoutAt cho tất cả phiên chưa đăng xuất
