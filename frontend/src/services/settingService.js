 ...
 */
export const revokeLoginSession = async (loginHistoryId) => {
  const response = await api.delete(`/settings/login-history/${loginHistoryId}`);
  return response.data;
};
...
export const revokeAllOtherSessions = async () => {
  const response = await api.post('/settings/revoke-all-sessions');
  return response.data;
};
