...
const ChatAIPopup = () => {
  ...
  const initialMessage = useCallback(() => {
    return {
      sender: 'ai',
      // [THAY ĐỔI] Tuân theo cấu trúc mới
      type: 'suggestions',
      text: `Chào ${user?.username || 'bạn'}, Trợ lý AI Sạp Phim có thể giúp gì cho bạn?`,
      payload: [
        { text: "Gợi ý phim hay", action_prompt: "Gợi ý phim hay" },
        { text: "Tìm phim mới nhất", action_prompt: "Phim mới nhất" },
      ],
  ...
  const handleSendMessage = useCallback(async (messageText) => {
    ...
    const newMessage = {
      sender: 'user',
      type: 'text', // Tin nhắn của user luôn là 'text'
      text: text,
      payload: null,
      timestamp: new Date().toISOString(),
    };
    ...
    try {
      const responseData = await aiService.chatWithAI(text);
      
      setMessages((prevMessages) => [
        ...prevMessages,
        {
          sender: 'ai',
          ...responseData, // responseData đã là { type, text, payload }
          timestamp: new Date().toISOString(),
        },
      ]);
    } catch (error) {
            ...
            <form className="chat-ai-popup-input-form" onSubmit={handleFormSubmit}>
              <div className="chat-input-wrapper">
                <input
                  ...
                  placeholder="Hỏi Trợ lý AI Sạp Phim..."
                  disabled={isLoading}
