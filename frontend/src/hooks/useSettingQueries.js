...
/**
 * Hook to revoke a specific login session
 */
export const useRevokeLoginSession = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (loginHistoryId) => settingService.revokeLoginSession(loginHistoryId),
    onSuccess: (response) => {
      queryClient.invalidateQueries(['loginHistory']);
      toast.success(response.message || 'Hủy phiên đăng nhập thành công!');
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Lỗi khi hủy phiên đăng nhập.');
    },
  });
};

/**
 * Hook to revoke all other login sessions
 */
export const useRevokeAllOtherSessions = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: () => settingService.revokeAllOtherSessions(),
    onSuccess: (response) => {
      queryClient.invalidateQueries(['loginHistory']);
      toast.success(response.message || 'Đã hủy tất cả phiên đăng nhập khác thành công!');
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Lỗi khi hủy phiên đăng nhập.');
    },
