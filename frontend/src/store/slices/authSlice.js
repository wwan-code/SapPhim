...
export const deleteUserAvatar = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const message = error.response?.data?.message || error.message || 'Xóa ảnh đại diện thất bại.';
      return rejectWithValue(message);
    }
  }
);

// Async action: Xóa ảnh bìa
export const deleteUserCover = createAsyncThunk(
  'user/deleteCover',
  async (_, { rejectWithValue }) => {
    try {
      ...
      const message = error.response?.data?.message || error.message || 'Xóa ảnh bìa thất bại.';
      return rejectWithValue(message);
    }
  }
);

// Async action: Cập nhật social links
export const updateUserSocialLinks = createAsyncThunk(
  'user/updateSocialLinks',
  async (socialLinks, { rejectWithValue }) => {
    try {
      const response = await userService.updateSocialLinks(socialLinks);
      return response.data.data;
    } catch (error) {
      const message = error.response?.data?.message || error.message || 'Cập nhật liên kết mạng xã hội thất bại.';
      return rejectWithValue(message);
    }
  }
);
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteUserCover.fulfilled, (state, action) => {
        state.loading = false;
        state.user = action.payload;
      })
